...
def run_pearson_correlation_analysis(X, verbose=True):
    """
    Run Pearson correlation analysis for causal discovery
    """
...
def run_spearman_correlation_analysis(X, verbose=True):
    """
    Run Spearman correlation analysis for causal discovery
    """
    n_samples, n_features = X.shape
...
def calculate_mutual_information(x, y, n_bins=10):
    """
    Calculate mutual information between two continuous variables using discretization
    """
...
def maximum_spanning_tree_from_similarity(similarity_matrix):
    ...
    for u, v, weight in tree_edges:
        ...
        tree_adjacency[v, u] = weight  # Undirected tree
    
    return tree_adjacency

def run_chowliu_analysis(X, verbose=True):
    """
    Run Chow-Liu algorithm for structure learning
    """
    n_samples, n_features = X.shape
    
    # Step 1: Calculate pairwise mutual information matrix
    ...
    # Step 2: Find maximum spanning tree using Chow-Liu algorithm
...
def load_dataset(dataset_name, dataset_version, verbose=True):
    ...
    D_df = data.frame  # features and target 

    X_df = data.data  # only take features, exclude label
    
    return data, D_df, X_df

def apply_simple_preprocessing(X_df, verbose=True):
    """Apply simple one-hot encoding preprocessing"""
    printer = VerbosePrinter(verbose)
    ...
    # Step 2: Handle numerical columns - drop rows with missing values
    ...
    # Step 3: Handle categorical columns with one-hot encoding
    X_categorical_encoded = pd.DataFrame(index=X_numerical.index)  # Match the index

    if categorical_cols:
        for col in categorical_cols:
            # Get categorical data matching numerical data index
            ...
            # Limit categories if too many unique values
    ...
    # Step 4: Combine numerical and encoded categorical features
...
def apply_robust_preprocessing(X_df, verbose=True):
    """Apply robust preprocessing for datasets with many missing values"""
    printer = VerbosePrinter(verbose)
    
    # Store original dimensions for accurate reporting
    ORIGINAL_OBSERVATIONS = X_df.shape[0]
    ORIGINAL_FEATURES = X_df.shape[1]

    def analyze_missing_data(df):
        """Comprehensive missing data analysis following academic standards"""
    ...
    def robust_data_type_conversion(df):
        """Convert all categorical and object columns to string type for robust handling"""
        df_processed = df.copy()
        
        # Convert categorical and object columns to string
        for col in df_processed.columns:
            if df_processed[col].dtype.name in ['category', 'object']:
                df_processed[col] = df_processed[col].astype('object')
                # Convert NaN to string 'NaN' for consistent handling
    ...
    # Step 1: Convert data types for robust processing
    ...
    # Step 3: Apply missing data strategy
    def apply_robust_missing_data_strategy(df_original, df_converted, numerical_cols, categorical_cols, missing_analysis):
        """Apply evidence-based missing data handling strategy with robust implementation"""
        
        # Strategy 1: Drop features with >70% missing (academic threshold)
        ...
        if not X_numerical.empty:
            # Check remaining missing percentage after feature dropping
            remaining_complete_cases = X_numerical.dropna().shape[0]
            remaining_percentage = remaining_complete_cases / X_numerical.shape[0] * 100
            
            # Decision rule based on remaining data
            if remaining_percentage >= 30:  # Academic threshold: keep if >=30% complete cases
                X_numerical = X_numerical.dropna()
            else:
                try:
    ...
    if categorical_cols and not X_numerical.empty:
        for col in categorical_cols:
            if col in X_df_processed.columns:  # Check if column wasn't dropped
                ...
                # Handle high cardinality (academic practice)
                n_unique = cat_data.nunique()
                max_categories = min(20, max(5, int(len(cat_data) * 0.1)))  # Academic rule: max 10% of observations, between 5-20
                
                if n_unique > max_categories:
                    value_counts = cat_data.value_counts()
                    top_categories = value_counts.head(max_categories-1).index.tolist()
                    
                    # Create a mask for top categories
                ...
                # One-hot encoding with robust error handling
                try:
                    ...
                    if hasattr(encoder, 'get_feature_names_out'):
                        ...
                        if len(categories) > 1:  # Because we used drop='first'
...
def preprocess_data(X_df, verbose=True):
    """Apply preprocessing with fallback to robust method"""
    try:
...
def run_dag_construction(dataset_name, dataset_version, method='lingam', thresholds=None, verbose=True):
    ...
    # Run DAG construction algorithm based on method
    results = {}
    ...
    if method == 'lingam':
        ...
        # Create stats compatible with LinGAM format
        best_stats = {
    ...
    def identify_original_features_correct(feature_names, numerical_cols, categorical_cols):
        """Correctly identify which original feature each node belongs to"""
        feature_mapping = {}
        original_features = {}
