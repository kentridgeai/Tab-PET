"""
LLPE (Laplacian Learned Positional Encodings) generation module
"""
...
def compute_laplacian_eigenvectors(A, k_first=5, k_last=5):
    """
    Compute normalized Laplacian eigenvectors following LLPE methodology
    """
    nnodes = A.shape[0]
    
    # Compute degree vector
    D_vec = np.sum(A, axis=1).flatten()
    D_vec[D_vec == 0] = 1  # Avoid division by zero for isolated nodes
...
def automatic_k_selection(A_symmetric):
    ...
    # Auto-select k_first with max constraint of 15
    max_first_k = 10
...
def apply_simple_pe_normalization(eigenvectors):
    """
    Apply simple normalization to eigenvectors without Chebyshev transform
    """
...
def run_llpe_generation(dag_results, dataset_name, dataset_version, verbose=True):
    """Main function to run LLPE generation"""
    ...
    # Convert DAGs to Graph Laplacian
