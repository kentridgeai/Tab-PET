"""
Main execution script for FT-Transformer with Tab-PET
"""
...
from ft_transformer_experiment import run_ft_transformer_experiment
...
def parse_arguments():
    ...
    parser.add_argument('--method', type=str, default=DAG_METHOD,
                       ...
                       help='DAG construction method (default: lingam)')
...
def main():
    ...
    printer.section("FT-TRANSFORMER WITH POSITIONAL ENCODINGS EXPERIMENT")
    printer.print(f"Dataset: {args.dataset_name} v{args.dataset_version}")
    printer.print(f"Method: {args.method}")
    if args.method == 'lingam':
        printer.print(f"LinGAM thresholds: {args.thresholds}")
    else:
        printer.print(f"NOTEARS lambda1 values: {args.thresholds}")
    printer.print(f"GPU ID: {args.gpu_id}")
    printer.print(f"Seeds: {args.seeds}")
    printer.print(f"PE alphas: {args.pe_alphas}")
    printer.print(f"Max epochs: {args.num_epochs}")
    printer.print(f"Early stopping: {args.early_stopping}")
    ...
    try:
        # Stage 1: Graph Estimation
        printer.section("STAGE 1: Graph Estimation")
        ...
        # Stage 2: PE Creation
        printer.section("STAGE 2: PE Creation")
        ...
        # Stage 3: FT-Transformer with Tab-PET Experiment
        printer.section("STAGE 3: FT-Transformer with Tab-PET Experiment")
        experiment_results = run_ft_transformer_experiment(
            dag_results=dag_results,
        ...
        printer.print(f"Results saved to CSV files")
